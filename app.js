...
const FaqItem = {
  ...
  setup(props, { emit }) {
    ...
    const isOpen = Vue.computed(() => props.activeItem === props.index);

    const toggleAccordion = () => {
      emit('toggle', props.index);
    };

    Vue.watch(isOpen, (newVal) => {
      if (answerRef.value) {
  ...
  setup() {
    ...
    const smoothScroll = async (event) => {
      ...
      if (targetSection) {
        ...
        if (dataSrc && !loadedContent.value[targetId]) {
    ...
    let holisticDevelopmentChart; // Chart.js instance
    ...
    const updateChartColors = () => {
      ...
      holisticDevelopmentChart.data.datasets[0].backgroundColor = secondaryColor + '80';
    ...
    // This function will be called when the student-development-content.html is loaded
    const createHolisticDevelopmentChart = () => {
    ...
    const loadSectionContent = async (sectionId, url) => {
        ...
        // Check if content is already loaded (using loadedContent ref)
        if (loadedContent.value[sectionId]) {
            console.log(`Content for ${sectionId} already loaded.`);
            return;
        }

        // Display loading message
        loadedContent.value[sectionId] = `<p>Loading content...</p>`;
        ...
        try {
            ...
            loadedContent.value[sectionId] = content; // Store content in reactive state
            
            await nextTick(); // Wait for Vue to update the DOM

            // Explicitly add 'visible' class to newly loaded fade-in elements
            const contentPlaceholder = sectionElement.querySelector('.container');
            ...
            // This part needs to be re-evaluated for Vue components.
            if (sectionId === 'faq') {
                // initializeFaqAccordions(); // This will be handled by a Vue FAQ component
            }
            if (sectionId === 'student-development-section') {
                createHolisticDevelopmentChart();
            }

        } catch (error) {
            console.error(`Could not load content for ${sectionId}:`, error);
            loadedContent.value[sectionId] = `<p>Failed to load content for this section. Please try again later.</p>`;
        }
    };
      ...
      const sectionObserver = new IntersectionObserver((entries, observer) => {
          entries.forEach(entry => {
              if (entry.isIntersecting) {
                  ...
                  console.log(`Section ${sectionId} is intersecting. Loading content...`);

                  if (dataSrc && !loadedContent.value[sectionId]) {
    ...
    return {
      ...
      createHolisticDevelopmentChart, // Expose to be called from outside if needed
      faqData // Expose FAQ data to the template
    };
  }
});

app.component('FaqSection', FaqSection); // Register FaqSection globally
